...
export function PrescriptionsView({
  ...
  data: AdminData;
  copy: any;
  onLog: (u: string, a: string, t: string) => void;
  toast: any;
}) {
  ...
  const validate = useCallback(
    (currentForm: Prescription, currentMeds: Medicine[], hasPatient: boolean): FieldError => {
      const e: FieldError = {};
  ...
  const save = async () => {
    ...
    const warnings = medicines
      .filter(m => allergies.some(allergy => m.name.toLowerCase().includes(allergy)))
      .map(m => `${m.name} may conflict with a recorded allergy.`);
    setInteractionWarnings(warnings);
    if (warnings.length > 0) {
      toast.show?.(warnings[0]);
      return;
    }
    ...
    const payload: Prescription = {
      ...
      auditTrail: [
        ...(form.auditTrail || []),
        {
          ...
          changes: `${medicines.length} medication(s), diagnosis updated`,
        },
      ],
    };
  ...
  const removeMedicine = (index: number) =>
    setMedicines(items => items.filter((_, i) => i !== index));
  ...
  const exportPdf = async (rx: Prescription) => {
    ...
    doc.text(
      [`Prescription ID: ${rx.id}`, `Date: ${rx.date}`, `Status: ${rx.status}`].join('  |  '),
      20,
      30
    );
    ...
    if (patient) {
      ...
      y += 6;
      doc.text(`ID: ${patient.id}`, 20, y);
      y += 6;
      doc.text(`Gender: ${patient.gender}  |  DOB: ${patient.dob}`, 20, y);
      y += 6;
    ...
    doc.setFontSize(10);
    doc.setTextColor(51, 78, 92);
    doc.text(`Diagnosis: ${rx.diagnosis}`, 20, y);
    y += 6;
    doc.text(`Doctor: ${rx.doctor}`, 20, y);
    y += 10;
    ...
    rx.medicines.forEach((med, i) => {
      ...
      y += 6;
      if (med.instructions) {
        doc.text(`Instructions: ${med.instructions}`, 24, y);
        y += 6;
      }
      y += 4;
    });
    ...
    <div className="rx-workspace">
      <div className="rx-heading">
        ...
        <div className="rx-actions">
          <button className="pro-outline btn-pro" onClick={() => window.print()}>
            <Printer size={15} /> {copy.rxPrint}
          </button>
          <button className="pro-primary btn-pro shadow-glow-teal" onClick={openCreate}>
            <Plus size={15} /> {copy.rxNew}
          </button>
        </div>
      </div>
      ...
      <div className="rx-layout-modern">
        <div className="pro-panel rx-toolbar">
          <div className="adm-filters">
            <div className="pro-search grow">
              <Search size={15} />
              <input
                placeholder={copy.rxSearch}
                value={query}
                onChange={e => setQuery(e.target.value)}
              />
            </div>
            <Select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}>
              {copy.rxFilters.map((f: string) => (
        ...
        {prescriptions.length === 0 ? (
          <div className="pro-panel rx-empty">
            <div className="rx-empty-art">
              <FileText size={32} />
            </div>
            <h3>{copy.rxNoPrescriptions}</h3>
            <p className="muted-light">{copy.rxNoPrescriptionsBody}</p>
            <button className="pro-primary" onClick={openCreate}>
              <Plus size={14} /> {copy.rxNew}
            </button>
          </div>
        ) : (
          <div className="rx-table-wrap">
            <table className="adm-table">
              <thead>
                <tr>
                  <th>{copy.rxPatient}</th>
                  <th>{copy.rxDate}</th>
                  <th>{copy.rxDiagnosis}</th>
                  <th>{copy.rxMedicines}</th>
                  <th>{copy.rxStatus}</th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {prescriptions.map((rx, i) => (
                  <tr
                    ...
                    <td>
                      <span className="muted-light">{rx.date}</span>
                    </td>
                    <td>
                      <span className="rx-diag">{rx.diagnosis}</span>
                    </td>
                    <td>
                      <span className="adm-pill adm-pill-neutral">
                        {rx.medicines.length} {copy.rxMedicines.toLowerCase()}
                      </span>
                    </td>
                    <td>{statusBadge(rx.status)}</td>
                    <td>
                      <div className="adm-row-actions" onClick={e => e.stopPropagation()}>
                        <button onClick={() => setViewingRx(rx)} title={copy.rxView}>
                          <FileText size={15} />
                        </button>
                        <button onClick={() => openEdit(rx)} title={copy.rxEdit}>
                          <Edit3 size={15} />
                        </button>
                        <button
                          className="danger"
                          onClick={() => setConfirmDelete(rx)}
                          title={copy.rxDelete}>
                          <Trash2 size={15} />
                        </button>
                      </div>
      ...
      {showForm && (
        <div className="adm-modal-backdrop" onClick={() => setShowForm(false)}>
          <div
            className="adm-modal adm-modal-lg rx-centered-modal"
            onClick={e => e.stopPropagation()}
            ref={formRef}
            role="dialog"
            aria-modal="true"
            aria-label={editingId ? copy.rxEdit : copy.rxNew}>
              ...
              <div className="rx-doc-header">
                <div className="rx-doc-brand">
                  ...
                  <div>
                    ...
                    <small>
                      {copy.doctor ||
                        'Dr. Ibrahim Hossain · M.Sc Skin & VD Integrative Medicine Consultant'}
                    </small>
                  </div>
                </div>
                <div className="rx-doc-meta">
                  <span>
                    Rx No: <strong>{editingId || nextRxId(data.prescriptions)}</strong>
                  </span>
                  <span>
              ...
              <div className="rx-doc-body">
                ...
                <div className="rx-grid">
                  <div className="rx-block">
                    <label>
                      {copy.rxPatientDetails}{' '}
                      {errors.patient && <em className="rx-error">Required</em>}
                    </label>
                      ...
                      {selectedPatient ? (
                        <>
                          <div className="rx-patient-row">
                            <span>
                              <small>{copy.rxFullName}</small>
                              <strong>{selectedPatient.name}</strong>
                            </span>
                            <span>
                              <small>{copy.rxPatientId}</small>
                              <strong>{selectedPatient.id}</strong>
                            </span>
                          </div>
                          <div className="rx-patient-row">
                            <span>
                              <small>{copy.rxGender}</small>
                              <strong>{selectedPatient.gender}</strong>
                            </span>
                            <span>
                              <small>{copy.rxDob}</small>
                              <strong>{selectedPatient.dob}</strong>
                            </span>
                          </div>
                          <div className="rx-patient-row">
                            <span>
                              <small>{copy.rxPhone}</small>
                              <strong>{selectedPatient.phone}</strong>
                            </span>
                            <span>
                              <small>{copy.rxBloodGroup}</small>
                              <strong>{selectedPatient.bloodGroup}</strong>
                            </span>
                          </div>
                          <div className="rx-patient-row">
                            <span className="rx-full">
                              <small>{copy.rxAddress}</small>
                        ...
                        <p className={`muted-light ${errors.patient ? 'rx-error-text' : ''}`}>
                          No patient selected
                        </p>
                      )}
                    </div>
                  ...
                  <div className="rx-block">
                    <label>
                      {copy.rxDiagnosisLabel}{' '}
                      {errors.diagnosis && <em className="rx-error">Required</em>}
                    </label>
                    <div className={`rx-diagnosis-box ${errors.diagnosis ? 'rx-field-error' : ''}`}>
                      <Input
                        id={errors.diagnosis ? firstErrorId : undefined}
                        value={form.diagnosis}
                        onChange={e => updateField('diagnosis', e.target.value)}
                        onBlur={() => touchedField('diagnosis')}
                        placeholder="e.g. Skin flare review"
                      />
                    </div>
                ...
                <div className="rx-medication-section">
                  <div className="rx-med-header">
                    ...
                    <button type="button" className="pro-text-link" onClick={addMedicine}>
                      <Plus size={14} /> {copy.rxAddMedicine}
                    </button>
                  </div>
                    ...
                    <table className="rx-med-table">
                      <thead>
                        <tr>
                          ...
                          <th>
                            {copy.rxMedicine}{' '}
                            {errors.medicineName && <em className="rx-error">Required</em>}
                          </th>
                          <th>{copy.rxDose}</th>
                      ...
                      <tbody>
                        {medicines.map((med, index) => (
                          <tr
                            key={index}
                            className={
                              med.name.trim() === '' && touched.medicineName ? 'rx-row-error' : ''
                            }>
                            <td>
                              <span className="rx-med-num">
                            ...
                            <td>
                              <Input
                                ...
                                onBlur={() => touchedField('medicineName')}
                ...
                <div className="rx-status-timeline">
                  {statusSteps.map((step, i) => (
                    <button
                      key={step}
                      type="button"
                      className={`${i <= currentStatusIndex ? 'is-complete' : ''}`}
              ...
              <button
                ...
                onClick={() => {
                  const rx = editingId
                    ...
                    : ({
                        ...form,
                        id: nextRxId(data.prescriptions),
                        medicines,
                        status: form.status || 'Draft',
                        createdAt: new Date().toISOString(),
                      } as Prescription);
                ...
                <Download size={14} /> {copy.rxExportWord}
              </button>
              <button
                className="pro-outline"
                onClick={async () => {
                ...
                <Download size={14} /> {copy.rxExportPdf}
              </button>
              <button className="pro-primary" onClick={save} disabled={isFormInvalid}>
                <Check size={14} /> {copy.rxSave}
              </button>
            </div>
          </div>
        </div>
          ...
          <div
            ...
            <div className="rx-document">
              <div className="rx-doc-header">
                ...
                <div className="rx-doc-meta">
                  <span>
                    Rx No: <strong>{viewingRx.id}</strong>
                  </span>
                  <span>
                    Date: <strong>{viewingRx.date}</strong>
                  </span>
                </div>
              </div>
              <div className="rx-doc-body">
                    ...
                    <div className="rx-grid">
                          ...
                          <div className="rx-patient-card">
                            ...
                            {patient.allergies.length > 0 && (
                              <div className="rx-patient-row">
                                <span className="rx-full">
                                  <small>{copy.rxAllergies}</small>
                                  <strong className="coral">{patient.allergies.join(', ')}</strong>
                                </span>
                              </div>
                            )}
                          </div>
                      ...
                      <div className="rx-block">
                        ...
                        <div className="rx-diagnosis-box">
                          <strong>{viewingRx.diagnosis}</strong>
                        </div>
                        <label style={{ marginTop: 14 }}>{copy.rxDoctor}</label>
                        <div className="rx-diagnosis-box">
                          <strong>{viewingRx.doctor}</strong>
                        </div>
                        <label style={{ marginTop: 14 }}>{copy.rxStatus}</label>
                        <div>
                          <span
                            className={`adm-pill adm-pill-${viewingRx.status === 'Signed' ? 'teal' : viewingRx.status === 'Sent' ? 'blue' : 'sand'}`}>
                            {viewingRx.status}
                          </span>
                        </div>
                      </div>
                    ...
                    <table className="rx-med-table">
                      ...
                      <tbody>
                        {viewingRx.medicines.map((m, i) => (
                          <tr key={i}>
                            <td>
                              <span className="rx-med-num">{String(i + 1).padStart(2, '0')}</span>
                            </td>
                            <td>
                ...
                {viewingRx.notes && (
                  <div className="rx-block" style={{ marginTop: 18 }}>
                    <label>{copy.rxNotes}</label>
                    <p style={{ color: '#516974', fontSize: 13, lineHeight: 1.6 }}>
                      {viewingRx.notes}
                    </p>
                  </div>
                )}
            ...
            <div className="adm-modal-foot">
              <button className="pro-outline" onClick={() => setViewingRx(null)}>
                {copy.close}
              </button>
              <button
                className="pro-outline"
                onClick={() => {
                  if (viewingRx) exportWord(viewingRx);
                }}>
                <Download size={14} /> {copy.rxExportWord}
              </button>
              <button
                ...
                onClick={() => {
                  if (viewingRx) exportPdf(viewingRx);
                }}>
                <Download size={14} /> {copy.rxExportPdf}
              </button>
              <button
                className="pro-outline"
                onClick={() => {
                  if (viewingRx) printRx(viewingRx);
                }}>
                <Printer size={14} /> {copy.rxPrint}
              </button>
            </div>
          </div>
        </div>
      ...
      {confirmDelete && (
        <Modal
          ...
          title={copy.rxConfirmDelete}
          footer={
            <>
              <button className="pro-outline" onClick={() => setConfirmDelete(null)}>
                {copy.cancel}
              </button>
              <button className="pro-danger" onClick={() => deleteRx(confirmDelete)}>
                <Trash2 size={14} /> {copy.delete}
              </button>
            </>
          }>
          <p>
            {copy.rxConfirmDeleteBody
              ?.replace('{id}', confirmDelete.id)
              .replace('{patient}', confirmDelete.patientName)}
          </p>
        </Modal>
      )}
    </div>
