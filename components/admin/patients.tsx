    ...
    <>
      <section className="adm-page-head">
        ...
        <div className="adm-head-actions">
          <button className="pro-outline" onClick={() => toast.show('Exporting…', 'info')}>
            <Download size={14} /> {copy.export}
          </button>
          <button className="pro-outline" onClick={() => toast.show('Import wizard', 'info')}>
            <Upload size={14} /> {copy.import}
          </button>
          <button
      ...
      <section className="pro-panel adm-toolbar">
        <div className="adm-filters">
          ...
          <Select value={filter} onChange={e => setFilter(e.target.value)}>
            ...
            <option>Active medications</option>
          </Select>
          <button
      ...
      {confirmDelete && (
        <Modal
          ...
          <p>
            Permanently remove <strong>{confirmDelete.name}</strong> ({confirmDelete.id}) and all
            related records?
          </p>
        </Modal>
      )}
...
function PatientForm({
  ...
  return (
    <Drawer
      open
      onClose={onClose}
      title={initial ? `Edit ${initial.name}` : 'New patient'}
      width={680}>
      <form
        className="adm-form"
        onSubmit={e => {
        ...
        <div className="adm-form-grid">
          ...
          <Field label="Gender">
            <Select value={p.gender} onChange={e => setP({ ...p, gender: e.target.value as any })}>
              <option>Female</option>
              <option>Male</option>
              <option>Other</option>
            </Select>
          ...
          <Field label="Blood group">
            <Select value={p.bloodGroup} onChange={e => setP({ ...p, bloodGroup: e.target.value })}>
          ...
          <div className="adm-tag-input">
            ...
            <div className="adm-tag-add">
              ...
              <button
                ...
                onClick={() => {
                  if (allergy.trim()) {
                    setP({ ...p, allergies: [...p.allergies, allergy.trim()] });
                    setAllergy('');
                  }
                }}>
                Add
          ...
          <div className="adm-tag-input">
            <div className="adm-tags">
              {p.conditions.map(c => (
                <Pill key={c} tone="blue">
                  ...
                  <button
                    type="button"
                    onClick={() => setP({ ...p, conditions: p.conditions.filter(x => x !== c) })}>
                    <X size={11} />
                  </button>
                </Pill>
              ))}
            ...
            <div className="adm-tag-add">
              ...
              <button
                ...
                onClick={() => {
                  if (condition.trim()) {
                    setP({ ...p, conditions: [...p.conditions, condition.trim()] });
                    setCondition('');
                  }
                }}>
                Add
...
function PatientProfile({
  ...
  const [tab, setTab] = useState<'overview' | 'visits' | 'prescriptions' | 'notes' | 'documents'>(
    'overview'
  );
      ...
      <div className="adm-profile">
        <div className="adm-profile-hero">
          ...
          <div className="adm-profile-meta">
            <h2>{p.name}</h2>
            <p>
              {p.gender} · {age} years · {p.bloodGroup}
            </p>
          ...
          <div className="adm-profile-actions">
            <button className="pro-outline" onClick={() => onEdit(p)}>
              <Edit3 size={14} /> {copy.edit}
            </button>
            <button className="pro-danger" onClick={() => onDelete(p)}>
              <Trash2 size={14} /> {copy.delete}
            </button>
          </div>
        </div>
        ...
        <div className="adm-profile-stats">
          ...
          <div>
            ...
            <strong>{p.medications.length}</strong>
          </div>
          <div>
            <small>Notes</small>
            <strong>{p.notes.length}</strong>
          </div>
          <div>
          ...
          <>
            ...
            <div className="adm-vitals">
              <div>
                ...
                <strong>{p.vitals.bp || '—'}</strong>
              </div>
              <div>
                <small>Heart rate</small>
                <strong>{p.vitals.hr || '—'}</strong>
              </div>
              <div>
                <small>Temperature</small>
                <strong>{p.vitals.temp || '—'}</strong>
              </div>
              <div>
                <small>Weight</small>
        ...
        {tab === 'visits' && (
          <>
            <div className="adm-section-head">
              <h4>Visit history</h4>
              <button className="pro-primary" onClick={() => setShowVisit(true)}>
                <Plus size={13} /> {copy.addVisit}
              </button>
            </div>
            {p.visits.length === 0 ? (
              <EmptyState title="No visits yet" body="Add the first visit to start a record." />
            ) : (
        ...
        {tab === 'prescriptions' && (
          <>
            <div className="adm-section-head">
              ...
              <button className="pro-primary" onClick={() => setShowRx(true)}>
                <Plus size={13} /> {copy.addPrescription}
              </button>
            </div>
            {p.medications.length === 0 ? (
              <EmptyState title="No medications" body="Add the patient's current prescriptions." />
            ) : (
              <ul className="adm-rx-list">
                {p.medications.map((m, i) => (
                  <li key={i}>
                    ...
                    <Pill tone={m.status === 'Ongoing' ? 'teal' : 'sand'}>{m.status}</Pill>
                  </li>
                ))}
              </ul>
            )}
        ...
        {tab === 'notes' && (
          <>
            <div className="adm-section-head">
              <h4>Doctor notes</h4>
              <button className="pro-primary" onClick={() => setShowNote(true)}>
                <Plus size={13} /> {copy.addNote}
              </button>
            </div>
            {p.notes.length === 0 ? (
        ...
        {tab === 'documents' && (
          <>
            ...
            {p.documents.length === 0 ? (
              <EmptyState title="No documents" body="Upload lab reports, scans, or other files." />
            ) : (
...
function VisitForm({ onClose, onSave }: { onClose: () => void; onSave: (v: any) => void }) {
  const [v, setV] = useState({ date: TODAY, reason: '', doctor: 'Dr. Ibrahim', notes: '' });
  return (
    <Modal
      open
      onClose={onClose}
      ...
      footer={
        <>
          ...
          <button className="pro-primary" onClick={() => onSave(v)}>
            <Save size={14} /> Save
          </button>
        </>
      }>
      <div className="adm-form-grid">
        ...
        <Field label="Doctor">
          <Input value={v.doctor} onChange={e => setV({ ...v, doctor: e.target.value })} />
        </Field>
      </div>
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <button className="pro-primary" onClick={() => onSave(r)}>
            <Save size={14} /> Save
          </button>
        </>
      }>
      ...
      <div className="adm-form-grid">
        <Field label="Dose">
          <Input
            value={r.dose}
            onChange={e => setR({ ...r, dose: e.target.value })}
...
function NoteForm({ onClose, onSave }: { onClose: () => void; onSave: (n: any) => void }) {
  const [n, setN] = useState({ date: TODAY, text: '', author: 'Dr. Ibrahim' });
  return (
    <Modal
      open
      onClose={onClose}
      ...
      footer={
        <>
          ...
          <button className="pro-primary" onClick={() => onSave(n)}>
            <Save size={14} /> Save
          </button>
        </>
      }>
      <Field label="Note" required>
        <Textarea rows={4} value={n.text} onChange={e => setN({ ...n, text: e.target.value })} />
      </Field>
    </Modal>
  );
}
