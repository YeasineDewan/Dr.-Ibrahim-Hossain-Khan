...
import { TrendingUp, Users, DollarSign, Calendar, ArrowUpRight, ArrowDownRight, Activity as ActivityIcon, MoreHorizontal } from 'lucide-react'
import { BarChart, Sparkline, Pill, Avatar } from '../admin-ui'

export function AnalyticsView({ copy }: { copy: any }) {
  const [range, setRange] = useState<'7d' | '30d' | '90d' | '1y'>('30d')
  const series = {
    '7d': { labels: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'], values: [12, 18, 14, 22, 19, 9, 6] },
  ...
  return (
    <>
      ...
      <div className="adm-stat-grid adm-stagger">
        <div className="adm-stat-card blue">
          <div className="adm-stat-card-head">
            <span><Calendar size={16}/> Appointments</span>
            <Pill tone="teal"><ArrowUpRight size={10}/> +12.4%</Pill>
          </div>
          <strong>1,284</strong>
        ...
        <div className="adm-stat-card teal">
          <div className="adm-stat-card-head">
            <span><Users size={16}/> New patients</span>
            <Pill tone="teal"><ArrowUpRight size={10}/> +8.1%</Pill>
          </div>
          <strong>342</strong>
        ...
        <div className="adm-stat-card gold">
          <div className="adm-stat-card-head">
            ...
            <Pill tone="gold"><ArrowUpRight size={10}/> +16.2%</Pill>
          </div>
          <strong>৳ 18.2L</strong>
        ...
        <div className="adm-stat-card coral">
          <div className="adm-stat-card-head">
            <span><ActivityIcon size={16}/> No-shows</span>
            <Pill tone="coral"><ArrowDownRight size={10}/> -3.1%</Pill>
          </div>
          <strong>4.2%</strong>
      ...
      <div className="pro-dashboard-grid adm-stagger">
        <section className="pro-panel">
          <div className="pro-panel-head">
            <div><span className="pro-kicker">APPOINTMENTS</span><h2>Volume by period</h2></div>
        ...
        <section className="pro-panel">
          <div className="pro-panel-head">
            <div><span className="pro-kicker">REVENUE</span><h2>By service</h2></div>
          </div>
          <ul className="adm-rank-list">
            {[
              ...
              <li key={r.name}>
                ...
                <div className="adm-bar-track"><span style={{ width: `${r.value}%`, background: r.color }}/></div>
      ...
      <div className="pro-dashboard-grid adm-stagger">
        <section className="pro-panel">
          <div className="pro-panel-head">
            <div><span className="pro-kicker">CHAMBERS</span><h2>Utilization</h2></div>
          </div>
        ...
        <section className="pro-panel">
          <div className="pro-panel-head">
            <div><span className="pro-kicker">TOP DOCTORS</span><h2>This month</h2></div>
          </div>
          <ul className="adm-rank-list">
            {[
              ...
              <li key={d.name}>
                ...
                <div className="adm-bar-track"><span style={{ width: `${(d.count / 248) * 100}%`, background: d.color }}/></div>
...
function DonutCard({ label, value, total, color }: { label: string; value: number; total: number; color: string }) {
  ...
  return (
    <div className="adm-donut-card">
      <svg viewBox="0 0 100 100">
        ...
        <circle cx="50" cy="50" r={r} fill="none" stroke={color} strokeWidth="8" strokeDasharray={c} strokeDashoffset={offset} strokeLinecap="round" transform="rotate(-90 50 50)"/>
      </svg>
...
export function ActivityLogView({ data, copy }: { data: any; copy: any }) {
  ...
  const list = filter === 'All' ? data.activity : data.activity.filter((a: any) => a.action === filter.toLowerCase())
  return (
    <>
      <section className="adm-page-head">
        <div>
          ...
          <h1>Activity log</h1>
          <p className="muted-light">A complete audit trail of changes made across the workspace.</p>
        </div>
        <div className="adm-segmented">
        ...
        <ul className="adm-timeline-list">
          {list.map((a: any, i: number) => (
            <li key={a.id} style={{ animationDelay: `${i * 40}ms` }}>
              ...
              <div className="grow">
                <div><strong>{a.user}</strong> <span className="adm-action">{a.action}</span> <strong>{a.target}</strong></div>
                <small>{a.time} · {a.ip}</small>
              </div>
              <Pill tone={a.action === 'flagged' ? 'coral' : a.action === 'approved' ? 'teal' : 'blue'}>{a.action}</Pill>
            </li>
          ))}
        </ul>
      </section>
