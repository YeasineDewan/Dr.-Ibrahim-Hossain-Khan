...
export function DashboardView({
  ...
  return (
    <>
      <section className="admin-welcome-row appear-down">
        <div>
          ...
          <h1 className="gradient-text">{copy.goodMorning}</h1>
          <p>{copy.dashboardSub}</p>
        </div>
        <div className="admin-quick-actions">
          <button
            className="pro-primary btn-pro shadow-glow-teal"
            onClick={() => onNavigate('Appointments')}>
            <Plus size={15} className="float-soft" /> {copy.newAppt}
          </button>
          <button className="pro-outline btn-pro" onClick={() => onNavigate('Patients')}>
            <Users size={15} /> {copy.addVisit}
          </button>
          <button
            className="pro-icon-button btn-pro"
      ...
      <section className="pulse-banner appear-up">
        <div>
          ...
          <p>
            {copy.pulseBody} · {overdue > 0 && <b className="coral">⚠ {overdue} overdue</b>}
            {unread > 0 && <b className="gold"> · {unread} notifications</b>}
          </p>
        </div>
        ...
        <div className="pulse-actions">
          <button className="pulse-cta btn-pro" onClick={() => onNavigate('Calendar')}>
            <CalendarDays size={15} /> {copy.viewSchedule}
          </button>
          <button
            className="pulse-cta primary btn-pro shadow-glow-teal"
            onClick={() => onNavigate('Appointments')}>
            <Plus size={15} /> {copy.newAppt}
          </button>
        </div>
      </section>
      ...
      <div className="pro-kpi-grid adm-stagger">
        {copy.kpis.map((k: any, i: number) => {
          ...
          const sparks = [
            [40, 55, 48, 70, 62, 80, 75, 90],
            [60, 70, 65, 80, 75, 88, 92, 95],
            [30, 45, 50, 60, 55, 70, 80, 85],
            [12, 9, 14, 8, 11, 7, 9, 5],
          ];
          const illustBg = [
            'rgba(20,184,166,0.1)',
          ...
          return (
            <button
              key={i}
              className="pro-kpi premium-card shine-card"
              onClick={() =>
                onNavigate(
                  i === 0
                    ? 'Appointments'
      ...
      <div className="pro-dashboard-grid adm-stagger">
        <section className="pro-panel schedule-panel">
          <div className="pro-panel-head">
            ...
            <div className="panel-head-actions">
              <button className="pro-pill-btn" onClick={() => onNavigate('Calendar')}>
                <CalendarDays size={13} /> {copy.calendar}
              </button>
          ...
          <div className="panel-foot">
            <button className="pro-text-link" onClick={() => onNavigate('Appointments')}>
        ...
        <section className="pro-panel">
          ...
          <BarChart
            values={[42, 58, 49, 71, 63, 82, 76, 91, 86, 96, 89, 100]}
            labels={['J', 'F', 'M', 'A', 'M', 'J', 'J', 'A', 'S', 'O', 'N', 'D']}
            color="#174b78"
          />
          <div className="perf-legend">
            <div>
              <span className="dot teal" /> {copy.appointmentsLabel}
            </div>
            <div>
              <span className="dot gold" /> {copy.revenueLabel}
            </div>
          </div>
        </section>
      </div>
      ...
      <div className="pro-dashboard-grid adm-stagger">
        <section className="pro-panel">
          <div className="pro-panel-head">
            <div>
              <span className="pro-kicker">{copy.patientFlowKicker}</span>
              <h2>{copy.patientFlowTitle}</h2>
            </div>
            <select className="pro-mini-select">
          ...
          <ul className="adm-list">
            <li>
              <span className="dot blue" /> {copy.returning} <strong>1,418</strong>
            </li>
            <li>
              <span className="dot teal" /> {copy.newPatients} <strong>666</strong>
            </li>
            <li>
              <span className="dot gold" /> {copy.averageVisits} <strong>3.2</strong>
            </li>
          </ul>
        </section>
        ...
        <section className="pro-panel">
          <div className="pro-panel-head">
            <div>
              <span className="pro-kicker">FOLLOW-UPS</span>
              <h2>Action required</h2>
            </div>
            <button className="pro-pill-btn" onClick={() => onNavigate('Follow-ups')}>
              View all
            </button>
          </div>
        ...
        <section className="pro-panel">
          <div className="pro-panel-head">
            <div>
              <span className="pro-kicker">RECENT ACTIVITY</span>
              <h2>Today</h2>
            </div>
            <button className="pro-pill-btn" onClick={() => onNavigate('Activity log')}>
              Full log
            </button>
          </div>
          <ul className="adm-activity-list">
            {data.activity.slice(0, 5).map(a => (
              <li key={a.id}>
                ...
                <div className="grow">
                  <strong>{a.user}</strong> <span className="muted-light">{a.action}</span>{' '}
