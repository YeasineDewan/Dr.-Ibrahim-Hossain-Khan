...
export function GalleryPage() {
  ...
  const videoSlides = videos.filter(video => video.status === 'Published').map(video => ({ title: video.title, meta: `${lang === 'bn' ? 'ক্লিনিক ভিডিও' : 'Clinic video'} • ${video.duration}`, image: video.thumbnail }))
  useEffect(() => {
          ...
          {step === 3 && (
            <>
              ...
              <div className="form-grid">
                {a.details.map((d, i) => ('textarea' in d ? <textarea key={i} placeholder={d.ph} className="glow-focus" value={details[`field-${i}`] || ''} onChange={e => setDetails(prev => ({ ...prev, [`field-${i}`]: e.target.value }))}/> : <input key={i} placeholder={d.ph} className="glow-focus" value={details[`field-${i}`] || ''} onChange={e => setDetails(prev => ({ ...prev, [`field-${i}`]: e.target.value }))}/>))}<input type="tel" placeholder={lang === 'bn' ? 'জরুরি যোগাযোগ নম্বর' : 'Emergency contact number'} className="glow-focus" value={details.emergency || ''} onChange={e => setDetails(prev => ({ ...prev, emergency: e.target.value }))}/><input type="file" accept="image/*,.pdf" className="glow-focus" onChange={e => setDetails(prev => ({ ...prev, report: e.target.files?.[0]?.name || '' }))}/>
              </div>
            </>
          )}
          {step === 4 && (
