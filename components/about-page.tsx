    ...
    <>
      ...
      <section className="section about-story reveal-section">
        <div className="container split-grid">
          <div className="appear-up">
            ...
            <h2>
              {a.philosophyTitle1} <em>{a.philosophyTitleEm}</em>
            </h2>
      ...
      <section className="section about-video reveal-section">
        <div className="container">
          <div className="section-heading">
            <div>
              <span className="pill pill-teal">{a.videoPill}</span>
              <h2>
                {a.videoTitle1} <em>{a.videoTitleEm}</em>
              </h2>
            </div>
      ...
      <section className="section about-details reveal-section">
        <div className="container details-grid">
          <div>
            ...
            <h2>
              {a.experienceTitle1} <em>{a.experienceTitleEm}</em>
            </h2>
            <div className="timeline">
              {a.timeline.map((tl, i) => (
                <div key={i}>
                  <strong>{tl.strong}</strong>
                  <span>{tl.span}</span>
                </div>
              ))}
            </div>
      ...
      <section className="section about-extra reveal-section">
        <div className="container">
          <div className="section-heading">
            <div>
              <span className="pill pill-teal">{a.journeyPill}</span>
              <h2>
                {a.journeyTitle1} <em>{a.journeyTitleEm}</em>
              </h2>
            </div>
      ...
      <section className="section faq-section reveal-section">
        <div className="container narrow">
          ...
          <h2>
            {a.faqTitle1} <em>{a.faqTitleEm}</em>
          </h2>
      ...
      <section className="cta-section">
        <div className="container cta-inner">
          <div>
            <span className="pill pill-teal">{a.ctaPill}</span>
            <h2>
              {a.ctaTitle1}
              <br />
              <em>{a.ctaTitleEm}</em>
...
function DoctorBioSection({
  ...
  bio: (typeof doctorBio)[keyof typeof doctorBio];
    ...
    <section className="section doctor-bio">
      <div className="container">
        ...
        <div className="bio-block">
          <h3>
            <BookOpen size={18} /> {bio.educationTitle}
          </h3>
          <ul className="bio-list">
            {bio.education.map((e, i) => (
              <li key={i}>
                <Check size={16} /> <span>{e}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="bio-block bio-philosophy">
          <h3>
            <Stethoscope size={18} /> {bio.philosophyTitle}
          </h3>
          <h4>{bio.philosophyHeading}</h4>
          <p>{bio.philosophyBody}</p>
          <p className="bio-quote">
            <Quote size={18} /> {bio.philosophyQuote}
          </p>
        </div>

        <div className="bio-block">
          <h3>
            <Sparkles size={18} /> {bio.interestTitle}
          </h3>
        ...
        <div className="bio-block">
          <h3>
            <HeartHandshake size={18} /> {bio.integrativeTitle}
          </h3>
          <p>{bio.integrativeBody}</p>
          <p>
            <strong>{lang === 'bn' ? 'লক্ষ্য: ' : 'Goal: '}</strong>
            {bio.integrativeGoal}
          </p>
        </div>
        ...
        <div className="bio-block">
          <h3>
            <ShieldCheck size={18} /> {bio.consultationTitle}
          </h3>
        ...
        <div className="bio-block bio-commitment">
          <h3>
            <HeartHandshake size={18} /> {bio.commitmentTitle}
          </h3>
          <ul className="bio-list">
            {bio.commitment.map((c, i) => (
              <li key={i}>
                <Check size={16} /> <span>{c}</span>
              </li>
            ))}
          </ul>
...
function SexualMedicineSection({
  copy,
  onNavigate,
}: {
  copy: (typeof sexualMedicineCopy)[keyof typeof sexualMedicineCopy];
  onNavigate: (p: string) => void;
}) {
  const { lang } = useLanguage();
  return (
