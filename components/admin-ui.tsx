...
export function Avatar({ name, size = 32 }: { name: string; size?: number }) {
  ...
  return (
    <span
      ...
      style={{ width: size, height: size, background: colors[i], fontSize: size * 0.36 }}>
      {initials}
    </span>
  );
}
...
export function Select(
  ...
  return (
    <select {...props} className={`adm-input adm-select ${props.className || ''}`}>
      {props.children}
    </select>
  );
}
