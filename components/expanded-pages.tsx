        ...
        <ScrollReveal>
          <div className="service-cta-card service-bottom">
            <div>
              ...
              <h2>
                {s.bottomTitle1} <em>{s.bottomTitleEm}</em>
              </h2>
            ...
            <Magnetic>
              <button
                ...
                {s.bottomBtn} <ArrowRight size={16} className="float-x" />
              </button>
            </Magnetic>
          </div>
        </ScrollReveal>
              ...
              <div className="contact-details grid-cards">
                <div className="contact-detail-card">
                  ...
                  <div style={{ marginTop: 12 }}>
                    <strong>{c.callHeading}</strong>
                    <p style={{ fontSize: 13, color: '#647985', margin: '4px 0 0' }}>
                      +880 1719 395 553
                    </p>
                    <small style={{ color: '#94a3b8', fontSize: 11 }}>{c.callHours}</small>
                  </div>
                </div>
                <div className="contact-detail-card">
                  <span
                  ...
                  <div style={{ marginTop: 12 }}>
                    ...
                    <p style={{ fontSize: 13, color: '#647985', margin: '4px 0 0' }}>
                      hello@dribrahim.clinic
                    </p>
                    <small style={{ color: '#94a3b8', fontSize: 11 }}>{c.emailHours}</small>
                  </div>
                </div>
                <div className="contact-detail-card">
                  <span
                  ...
                  <div style={{ marginTop: 12 }}>
                    <strong>{c.visitHeading}</strong>
                    <p style={{ fontSize: 13, color: '#647985', margin: '4px 0 0' }}>
                      {c.visitSub}
                    </p>
                    <small style={{ color: '#94a3b8', fontSize: 11 }}>{c.visitAddress}</small>
                  </div>
                </div>
              </div>
            </div>
            <div className="contact-form-card premium-card" style={{ background: '#fff' }}>
              {sent ? (
                <div className="contact-success">
                  ...
                  <button className="btn btn-outline btn-pro" onClick={() => setSent(false)}>
                    {cm.sendAnother}
                  </button>
                </div>
              ) : (
                <>
                  ...
                  <div className="contact-form">
                    <label>
                      {c.namePh.replace('Full name', cm.fullName)}
                      <input placeholder={c.namePh} className="glow-focus" />
                    </label>
                    <label>
                      {c.emailHeading.replace('Email us', cm.emailAddress)}
                      <input type="email" placeholder={c.emailPh} className="glow-focus" />
                    </label>
                    <label>
                      {cm.phoneNumber}
                      <input placeholder={c.phonePh} className="glow-focus" />
                    </label>
                    <label>
                      {c.topicLabel}
                      <select className="glow-focus">
                        {c.topics.map(o => (
                    ...
                    <label className="wide">
                      {c.messageLabel}
                      <textarea placeholder={c.messagePh} className="glow-focus" />
                    </label>
                    <button
                      className="btn btn-primary wide btn-pro shadow-glow-teal btn-tilt"
                      onClick={() => setSent(true)}>
                      {c.sendBtn} <Send size={15} className="float-x" />
                    </button>
                  </div>
                </>
              )}
        ...
        <div className="chamber-map-grid grid-cards">
          <article className="premium-card lift shine-card" style={{ padding: 18 }}>
            <div
              ...
              <MapPin size={32} className="float-soft" />
            </div>
            ...
            <a
              ...
              href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent('House 45, Road 22, Dhanmondi, Dhaka')}`}
              target="_blank"
              rel="noreferrer">
